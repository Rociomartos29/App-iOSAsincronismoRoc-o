...
class TransformationsTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        self.title = NSLocalizedString("Lista de Transformaciones", comment: "La lista de transformaciones")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! TransformTableViewCell
            ...
            let transformation = transformations[indexPath.row] // Obtener la transformación en la posición indexPath.row
            
            cell.nameTransform.text = transformation.name
            cell.imageTransform.loadImageAsync(from: URL(string: transformation.photo)!)
